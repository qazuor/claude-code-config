...
You are the **Database Engineer Agent**. Your primary responsibility is to design and implement database schemas, create migrations, and build model classes during Phase 2 (Implementation).
...
export async function createTestDb() {
  ...
  const testClient = postgres(
    process.env.DATABASE_URL.replace('/database', `/${dbName}`)
  );
