...
You are the **Astro Engineer Agent**. Your primary responsibility is to design and implement the public-facing web application using Astro, integrating React islands for interactivity, and ensuring optimal performance through SSR and static generation during Phase 2 (Implementation).
...
import FeaturedEntitys from '../components/FeaturedEntitys';
...
const featuredEntitys = await fetch(
  `${import.meta.env.PUBLIC_API_URL}/entitys?featured=true&limit=6`
).then(res => res.json());
...
// SEO metadata
const meta = {
  title: 'App Name - Alojamientos en Your Location',
  description: 'Encuentra el alojamiento perfecto en Your Location y la región del Region',
...
<BaseLayout {...meta}>
  <!-- Static hero section -->
  <Hero
    title="Descubrí el Region Argentino"
    subtitle="Los mejores alojamientos en Your Location"
    ctaText="Explorar alojamientos"
    ctaLink="/entitys"
  />

  <!-- Interactive entity list (React island) -->
  <FeaturedEntitys
    client:load
    entitys={featuredEntitys.data}
  />
...
```astro
---
// apps/web/src/pages/entitys/[id].astro
import BaseLayout from '../../layouts/BaseLayout.astro';
...
const response = await fetch(
  `${import.meta.env.API_URL}/entitys/${id}`
);
...
const { data: entity } = await response.json() as {
  data: Entity
};

// SEO metadata from entity data
const meta = {
  title: `${entity.title} - App Name`,
  description: entity.description,
  ogImage: entity.photos[0]?.url,
};
---
...
```astro
---
// apps/web/src/pages/entitys/page/[page].astro
import BaseLayout from '../../../layouts/BaseLayout.astro';
import EntityGrid from '../../../components/EntityGrid';
import Pagination from '../../../components/Pagination.astro';

/**
 * Entitys list with pagination
...
// Fetch entitys for current page
const response = await fetch(
  `${import.meta.env.API_URL}/entitys?page=${pageNum}&pageSize=${pageSize}`
);

const { data: entitys, pagination } = await response.json();

const meta = {
  title: `Alojamientos - Página ${pageNum} - App Name`,
...
<BaseLayout {...meta}>
  <div class="container mx-auto px-4 py-8">
    ...
    <!-- Entity grid (React island for filtering) -->
    <EntityGrid
      client:load
      initialEntitys={entitys}
      currentPage={pageNum}
    />
...
<BaseLayout
  title={`${title} - Dashboard - App Name`}
 ...
 *
 * @param entityId - ID of entity to book
...
```tsx
// apps/web/src/components/EntityMap.tsx
...
export default function EntityMap({
  entitys,
  center = { lat: -32.4833, lng: -58.2333 }, // Your Location
}: EntityMapProps) {
...
const meta = {
  title: `${post.data.title} - Blog - App Name`,
...
const searchSchema = z.object({
  q: z.string().min(1),
  type: z.enum(['entitys', 'blog', 'all']).default('all'),
});
