...
> {
  constructor(ctx: ServiceContext, model?: EntityModel) {
    super(ctx, model ?? new EntityModel());
  }
}
...
app.post('/entitys', async (c) => {
  ...
  const result = await db.insert(entitys).values(data);
 ...
 * @param input.entity - The entity being booked
...
describe('EntityService', () => {
  describe('create', () => {
    ...
    it('should fail with missing required fields', async () => {
      // Arrange
      const input = { title: 'Beach House' } as CreateEntityInput;
...
1. `EntityService.calculateSeasonalPrice()` - No tests
...
- ✗ Missing index on `entitys.owner_id`
- ✓ Pagination implemented correctly

### Recommendations

1. Add eager loading for booking details query
2. Add composite index: `CREATE INDEX idx_entitys_owner ON entitys(owner_id)`
```
...
- ✗ Missing authorization check in `DELETE /entitys/:id`
- ✓ Sensitive data properly redacted in logs

### Critical Issues

1. Authorization bypass in entity deletion endpoint
   - Location: `routes/entitys.ts:156`
...
export class EntityModel extends BaseModel<Entity> {
  ...
  async findByOwner(ownerId: string): Promise<Entity[]> {
    return this.db
      .select()
      .from(this.table)
...
2. **Missing index** on bookings.entity_id
   - **Location:** Database schema
   - **Issue:** N+1 query in listing entitys with bookings
