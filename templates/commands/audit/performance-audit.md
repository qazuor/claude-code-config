...
🔴 Critical Issues (2)
  1. N+1 Query in Entity Listing
     Location: services/entity.service.ts:145
...
API:
  ...
  ✗ Large payloads: /api/entitys (250KB)
  ✓ Compression enabled
...
### PERF-001: N+1 Query in Entity Listing
- **Severity**: Critical
- **Location**: services/entity.service.ts:145
- **Impact**:
...
- **Fix**:
  ```typescript
  // Before
  const entitys = await this.model.findAll();
  // After
  const entitys = await this.model.findAll({
    with: {
      owner: true,
