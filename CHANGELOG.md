...
- **Project name validation**: Allow dots (`.`) in project names and repository names to support domain-style names like `myproject.com`.

## [0.6.4] - 2025-12-26

### Fixed

- **Git tests CI failure**: Added git user configuration (`user.name` and `user.email`) to all tests that make commits, fixing CI failures in environments without global git config.

## [0.6.3] - 2025-12-26

### Fixed

- **Permission patterns**: Updated all bash permission patterns to use `:*` prefix matching syntax required by latest Claude Code (e.g., `Bash(git status:*)` instead of `Bash(git status*)`).
